...
public class ADCreate extends LdapModifyOperation {
    ...
    private Uid executeImpl() throws NamingException {
        ...
        if (LOG.isOk()) {
            LOG.info("------> Bundles logic create - oclass :{0} -- attrs :{1}", oclass.getObjectClassValue(), adAttrs.toString());
        ...
        LOG.info("------> back uid , object can set :{0}", OBJECTGUID.equals(conn.getSchemaMapping().getLdapUidAttribute(oclass)));
